...
$persona = new Persona();
$usuario = new Usuario();
$persona_res = $usuario_res = $alumno_res = "";
...
if ($apellido && $nombres && $dni && $domicilio && $fecha_nacimiento && 
    ...
    if ($alumno_id) {
      $alumno_res = $alumno->save(["id"=>$alumno_id,"dni"=>$dni, "apellido"=>$apellido, "nombre"=>$nombres, "anio_ingreso"=>date('Y'),
                                   "debe_titulo"=>'No', "habilitado"=>'Si',"idPersona"=>$persona_res]);
    } else {
      $alumno_res = $alumno->save(["dni"=>$dni, "apellido"=>$apellido, "nombre"=>$nombres, "anio_ingreso"=>date('Y'),
                                   "debe_titulo"=>'No', "habilitado"=>'Si',"idPersona"=>$persona_res]);
    };

    if ($persona_res && $usuario_res && $alumno_res) {
