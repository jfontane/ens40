...
abstract class SanitizeVars {
   ...
   //VALIDA SIN REEMPLAZAR
    public static function USUARIO($string, $min='', $max=''){
    ...
    public static function STRING_NUMBER_AND_LETTERS_AND_SPACES($strNumber){
       $patron = "/^[a-zA-Z0-9_ ]+$/";
    ...
    //VALIDA SIN REEMPLAZAR
     public static function STRING2($string, $min='', $max=''){
         if (preg_match("/^[a-zA-Z0-9ñÑ]*$/", $string)) {
         $len = strlen($string);
         if((($min != '') && ($len < $min)) || (($max != '') && ($len > $max)))
         return FALSE;
         return $string;
  ...
  //echo limpiarTags("este código es malicioso <script>alert('hola!')</script>");
    ...
    public static function NUMEROS($string, $min='', $max=''){
    ...
    public static function FECHA($string){
      $date = explode('-',$string);
